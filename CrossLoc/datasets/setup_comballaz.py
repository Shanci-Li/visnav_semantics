...
def main(args):
    ...
    winter_drone_data_cesium_ls, winter_drone_data_real_ls = [], []  # winter data
    for root, dirs, files in os.walk(args.matching_dir):
        if len(files):
            if os.path.basename(root).endswith('-sim'):
                ...
                elif 'winter' in os.path.basename(root):
                    winter_drone_data_cesium_ls.append(root)
                    winter_drone_data_real_ls.append(root_real)
                else:
    ...
    for cesium_path, real_path in zip(winter_drone_data_cesium_ls, winter_drone_data_real_ls):
        process_folder(cesium_path, None, dst_dir, 'winter_drone_sim', origin, args.stride, args.ignore_3d_label,
                       args.force_semantics_downsampling)
        process_folder(cesium_path, real_path, dst_dir, 'winter_drone_real', origin, args.stride, args.ignore_3d_label,
                       args.force_semantics_downsampling)

    # Split data into training/validation/testing
    for mode in ['lhs_sim', 'drone_sim', 'drone_real', 'oop_drone_sim', 'oop_drone_real',
                 'winter_drone_sim', 'winter_drone_real']:
        print('\n===== Splitting data in {:s} mode ====='.format(mode))
        split_data(os.path.join(dst_dir, mode), mode, 'comballaz', args.ignore_3d_label)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lhs_dir', type=str, default='comballaz-LHS',
    ...
    parser.add_argument('--output_dir', type=str,
                        ...
                        help='Destination directory for the organized data set. Use DSAC* convention.')
