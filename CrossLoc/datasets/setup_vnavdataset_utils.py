...
def _mp_func_coor(dat_src_path, dat_path, origin, stride, mp_counter, mp_lock, ttl_size):
    ...
    pc = pc[range(stride // 2, height, stride), :, :]
    pc = pc[:, range(stride // 2, width, stride), :].transpose((2, 0, 1))  # [3, H_ds, W_ds]
...
def _mp_func_pose(img_path, poses, txt_path, origin, mp_counter, mp_lock, ttl_size):
    ...
    idx = int(img_path.split('_')[1])   # true index
...
def _mp_func_semantics(in_semantics_path, out_semantics_path, stride, mp_counter, mp_lock, ttl_size):
    """Multiprocessing backbone function for semantics extraction"""
    if stride == 1:
        if os.path.exists(out_semantics_path):
            os.remove(out_semantics_path)
        os.symlink(in_semantics_path, out_semantics_path)
    else:
        semantics_map = np.load(in_semantics_path)  # [H, W] = [480, 720]
        assert semantics_map.shape[0] == height and semantics_map.shape[1] == width
        semantics_map = semantics_map[range(stride // 2, height, stride), :]
        semantics_map = semantics_map[:, range(stride // 2, width, stride)]  # [H_ds, W_ds]
        assert semantics_map.min() >= 0 and semantics_map.max() <= 255  # uint8 sanity check
        np.save(out_semantics_path, semantics_map.astype(np.uint8))  # [H_ds, W_ds] = [60, 90], save numpy array
    with mp_lock:
        mp_counter.value += 1
        print("\rSemantics extraction progress: {:d}/{:d}".format(mp_counter.value, ttl_size), end="", flush=True)


def process_folder(src_cesium, src_other, dst, mode, origin, stride, ignore_3d_label, force_semantics_downsampling):
    """Process raw data for each part per folder"""
    # mode in ['lhs_sim', 'drone_sim', 'drone_real', '*_drone_sim', '*_drone_real']
    ...
    if src_other is not None:
        ...
        assert len(image_cesium_ls) == len(image_other_ls), 'Image size in synthetic and non-synthetic set ' \
                                                            'are not the same!'
    else:
        image_other_ls = None

    pc_ls = sorted([os.path.join(src_cesium, path) for path in os.listdir(src_cesium) if path.endswith('_pc.npy')])
    semantics_ls = sorted([os.path.join(src_cesium, path) for path in os.listdir(src_cesium) if path.endswith('_semantics.npy')])
    ...
    if ignore_3d_label:
        sub_dirs = ['calibration', 'poses', 'rgb', 'semantics']
    else:
        sub_dirs = ['calibration', 'init', 'poses', 'rgb', 'depth', 'normal', 'semantics']
    for sub_dir in sub_dirs:
        ...
        if 'calibration' in sub_dir:
            ...
            for _, pc_path in enumerate(tqdm(pc_ls)):
                ...
                if mode.endswith('drone_sim') or mode == 'lhs_sim':
                    if mp_func_args_ls is None:
                        mp_func_args_ls = []
                    mp_func_args_ls.append((dat_src_path, dat_path, origin, stride, mp_counter, mp_lock, len(pc_ls)))
                else:
                    src_path = os.path.join(dst, mode.replace('drone_real', 'drone_sim'),
                                            sub_dir, os.path.basename(dat_path))
            ...
            if mp_func_args_ls is not None:
                with mp.Pool() as pool:
                    pool.starmap(_mp_func_coor, mp_func_args_ls, chunksize=1)
                print("")
            ...
            for _, img_path in enumerate(tqdm(image_cesium_ls)):
                ...
                if mode.endswith('drone_sim') or mode == 'lhs_sim':
                    ...
                    src_path = os.path.join(dst, mode.replace('drone_real', 'drone_sim'),
                                            sub_dir, os.path.basename(txt_path))
                    assert os.path.exists(src_path)
            ...
            if mp_func_args_ls is not None:
                with mp.Pool() as pool:
                    pool.starmap(_mp_func_pose, mp_func_args_ls, chunksize=1)
                print("")
        elif 'rgb' in sub_dir:
        ...
        elif 'semantics' in sub_dir:
            # use symbolic link to refer to actual dataset
            print('===== Preparing semantics folder ======')
            mp_func_args_ls = None
            with mp_lock:
                mp_counter.value = 0
            for _, semantics_path in enumerate(tqdm(semantics_ls)):
                src_path = semantics_path
                target_path = os.path.join(cur_path, os.path.basename(src_path)).replace('_semantics.npy', '.npy')

                # avoid duplication for non-synthetic matching datasets
                if mode.endswith('drone_sim') or mode == 'lhs_sim':
                    if mp_func_args_ls is None:
                        mp_func_args_ls = []
                    stride_sm = stride if force_semantics_downsampling else 1
                    mp_func_args_ls.append((src_path, target_path, stride_sm, mp_counter, mp_lock, len(pc_ls)))
                else:
                    src_path = os.path.join(dst, mode.replace('drone_real', 'drone_sim'),
                                            sub_dir, os.path.basename(target_path))
                    assert os.path.exists(src_path)
                    if os.path.exists(target_path):
                        os.remove(target_path)
                    os.symlink(src_path, target_path)
            if mp_func_args_ls is not None:
                with mp.Pool() as pool:
                    pool.starmap(_mp_func_semantics, mp_func_args_ls, chunksize=1)
                print("")
            ...
            for _, init_path in enumerate(tqdm(pc_ls)):
                ...
                if mode.endswith('drone_sim') or mode == 'lhs_sim':
                    ...
                    src_path = os.path.join(dst, mode.replace('drone_real', 'drone_sim'),
                                            sub_dir, os.path.basename(depth_path))
                    assert os.path.exists(src_path)
            ...
            if mp_func_args_ls is not None:
                with mp.Pool() as pool:
                    pool.starmap(_mp_func_depth, mp_func_args_ls, chunksize=1)
                print("")
            ...
            for _, init_path in enumerate(tqdm(pc_ls)):
                ...
                if mode.endswith('drone_sim') or mode == 'lhs_sim':
                    ...
                    src_path = os.path.join(dst, mode.replace('drone_real', 'drone_sim'),
                                            sub_dir, os.path.basename(normal_path))
                    assert os.path.exists(src_path)
            ...
            if mp_func_args_ls is not None:
                with mp.Pool() as pool:
                    pool.starmap(_mp_func_normal, mp_func_args_ls, chunksize=1)
                print("")


def split_data(base_dir, mode, dataset_name, ignore_3d_label=False):
    ...
    elif mode.endswith('drone_sim'):  # trajectory-dataset generated from Cesium
        split_ratio_ls = [0.4, 0.1, 0.5]
        nm_new_set_ls = ['train_' + mode, 'val_' + mode, 'test_' + mode]
        split_record = '{:s}_{:s}_split.txt'.format(dataset_name, mode.replace('_sim', ''))
    elif mode.endswith('drone_real'):  # trajectory-dataset taken by drone camera (w/ pre-processing)
        split_ratio_ls = [0.4, 0.1, 0.5]
        nm_new_set_ls = ['train_' + mode, 'val_' + mode, 'test_' + mode]
        split_record = '{:s}_{:s}_split.txt'.format(dataset_name, mode.replace('_real', ''))
        symlink_re_register = True
        nm_re_register_ls = ['train_' + mode.replace('drone_real', 'drone_sim'),
                             'val_' + mode.replace('drone_real', 'drone_sim'),
                             'test_' + mode.replace('drone_real', 'drone_sim')]
    else:
        raise NotImplementedError

    # completeness check
    if ignore_3d_label:
        len_semantics = len(os.listdir(os.path.join(base_dir, 'semantics')))
        len_rgb = len(os.listdir(os.path.join(base_dir, 'rgb')))
        assert len_semantics == len_rgb
    else:
    ...
    if not os.path.exists(split_record):
        ...
        for i, ratio in enumerate(split_ratio_ls):
            if i < len(split_ratio_ls) - 1:
                size_new_set_ls.append(int(len_rgb * ratio))
            else:
                size_new_set_ls.append(int(len_rgb - np.sum(size_new_set_ls)))
        assert np.sum(size_new_set_ls) == len_rgb

        print('{:d} images are found in {:s}. Will be split into following subsets:'.format(len_rgb, base_dir))
        [print("subset: {:s}, size: {:d}".format(nm, sz)) for nm, sz in zip(nm_new_set_ls, size_new_set_ls)]

        # generate subset indices randomly
        whole_idx_ls = np.random.permutation(np.arange(len_rgb))
        ...
        # sanity check
        assert len(np.concatenate(idx_new_set_ls)) == len_rgb
        ...
        # sanity check
        assert len(np.concatenate(idx_new_set_ls)) == len_rgb
        assert np.array_equal(np.sort(np.concatenate(idx_new_set_ls)), np.arange(len_rgb))
        for idx_set_a, idx_set_b in itertools.combinations(idx_new_set_ls, 2):
            assert len(np.intersect1d(idx_set_a, idx_set_b)) == 0
    ...
    for i, (set_name, set_filenames) in enumerate(zip(nm_new_set_ls, filenames_new_set_ls)):
        ...
        for folder in sub_dirs:
            if symlink_re_register and folder in ['init', 'poses', 'depth', 'normal', 'semantics']:
...
def virtual_merge_sections(src_dir_ls, dst_dir, ignore_3d_label=False):
    """
    ...
    @param ignore_3d_label: ignore 3d labels
    """
    ...
    mkdir(dst_dir)
    if ignore_3d_label:
        sub_dir_ls = ['calibration', 'poses', 'rgb', 'semantics']
    else:
        sub_dir_ls = ['calibration', 'init', 'poses', 'rgb', 'depth', 'normal', 'semantics']
    for src_dir in src_dir_ls:
        for sub_dir in sub_dir_ls:
