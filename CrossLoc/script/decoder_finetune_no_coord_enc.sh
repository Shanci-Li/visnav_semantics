...
if [ "$PT_REAL_DATA_DOM" == "in_place" ];
  then
    SP_SESSION='load-pt'$PT_SIM_DATA_CHUNK-ip-ft${PT_REAL_DATA_CHUNK}
  else
    SP_SESSION='load-pt'$PT_SIM_DATA_CHUNK-oop-ft${PT_REAL_DATA_CHUNK}
fi
    ...
    case $NET_DEPTH in
      "TINY")
        python3 finetune_decoder_single_task.py ${DATASET} --task ${TASK} \
          ...
          --encoders coord depth normal --session "${SP_SESSION}" --no_lr_scheduling
        ;;
      "FULL")
        python3 finetune_decoder_single_task.py ${DATASET} --task ${TASK} \
          --learningrate ${LR} --epochs ${EPOCHS} --inittolerance ${SCR_TOL}  --batch_size ${BATCH_SIZE} \
