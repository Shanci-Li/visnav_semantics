...
ENC_FT_COORD=$PROJ_DIR/weights/encoders-special/${DATASET}/pairwise-only/coord/model-rc${PT_REAL_DATA_CHUNK}.net
ENC_FT_DEPTH=$PROJ_DIR/weights/encoders-special/${DATASET}/pairwise-only/depth/model-rc${PT_REAL_DATA_CHUNK}.net
ENC_FT_NORMAL=$PROJ_DIR/weights/encoders-special/${DATASET}/pairwise-only/normal/model-rc${PT_REAL_DATA_CHUNK}.net

SP_SESSION=''
if [ "$PT_REAL_DATA_DOM" == "in_place" ];
  then
    SP_SESSION='load-pairwise-only'-ip-ft${PT_REAL_DATA_CHUNK}
  else
    SP_SESSION='load-pairwise-only'-oop-ft${PT_REAL_DATA_CHUNK}
fi
    ...
    case $NET_DEPTH in
      "TINY")
        python3 finetune_decoder_single_task.py ${DATASET} --task ${TASK} \
          ...
          --coord_weight ${ENC_FT_COORD}  --depth_weight ${ENC_FT_DEPTH}  --normal_weight ${ENC_FT_NORMAL}  --semantics_weight none \
