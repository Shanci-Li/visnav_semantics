...
then
    if [ $TASK == "coord" ]
    then
      echo "Use more epochs for out_of_place coord network finetuning: 150 -> 500"
      EPOCHS=500
    else
      echo "Use more epochs for out_of_place non-coord network finetuning: 150 -> 300"
      EPOCHS=300
    fi
fi
    ...
    case $NET_DEPTH in
      "TINY")
        python3 train_single_task.py ${DATASET} --task ${TASK} \
          ...
          --network_in ${ENC_PRETRAINED}  --session "${SP_SESSION}"
        ;;
      "FULL")
        python3 train_single_task.py ${DATASET} --task ${TASK} \
          --learningrate ${LR} --epochs ${EPOCHS} --inittolerance ${SCR_TOL}  --batch_size ${BATCH_SIZE} \
