...
then
    ...
    then
      echo "Use more epochs for out_of_place + urbanscape network finetuning: 150 -> 950"
      EPOCHS=950
    else
      echo "Use more epochs for out_of_place + naturescape network finetuning: 150 -> 2250"
      EPOCHS=2250
    fi
fi
...
then
    ...
    then
      ...
      echo "Use more epochs for in_place + naturescape network finetuning: 150 -> 600"
      EPOCHS=600
    fi
fi
...
then
    if [ $TASK == "coord" ]
    then
      echo "Use super 3000 long epochs for OOP data coord encoder fine-tuning!"
      EPOCHS=3000
    fi
fi
...
ENC_PRETRAINED=$PROJ_DIR/weights/encoders-pretrained/${DATASET}/${TASK}/model-sc-${SIM_DATA_CHUNK}.net
    ...
    case $NET_DEPTH in
      "TINY")
        python3 train_single_task.py ${DATASET} --task ${TASK} \
          ...
          --network_in ${ENC_PRETRAINED}  --session "${SP_SESSION}"  --no_lr_scheduling
        ;;
      "FULL")
        python3 train_single_task.py ${DATASET} --task ${TASK} \
          --learningrate ${LR} --epochs ${EPOCHS} --inittolerance ${SCR_TOL}  --batch_size ${BATCH_SIZE} \
