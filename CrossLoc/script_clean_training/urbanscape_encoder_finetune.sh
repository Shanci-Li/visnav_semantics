...
then
    ...
    then
      ...
      echo "Use more epochs for out_of_place + naturescape network finetuning: 150 -> 2500"
      EPOCHS=2500
    fi
fi
...
then
    if [ $DATASET == "urbanscape" ]
    then
      echo "Use more epochs for in_place + urbanscape network finetuning: 150 -> 800"
      EPOCHS=800
    else
...
ENC_PRETRAINED=$PROJ_DIR/weights-clean/encoders-pretrained/${DATASET}/${TASK}/model-sc-${SIM_DATA_CHUNK}.net
