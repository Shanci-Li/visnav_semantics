...
def config_dataloader(scene, task, grayscale, real_data_domain, real_data_chunk, sim_data_chunk,
    ...
    if real_data_chunk == 0.0:
        ...
        if not real_only:
            logging.info("This training uses pairwise sim-to-real data. {:d} iterations per epoch.".format(len(trainset)))
        else:
    ...
    if encoders_in is not None:
        ...
        for i, this_network_in in enumerate(encoders_in):
            if i == 0:
                ...
                if flag_loading:
                    _load_module(network.decoder, torch.load(this_network_in), prefix='decoder.')  # coord for decoder init.
                if flag_reuse_coord_encoder:
