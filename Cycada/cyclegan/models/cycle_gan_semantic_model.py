...
import torch
import itertools
from Cycada.cyclegan.util.image_pool import ImagePool
...
class CycleGANSemanticModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf,
                                            ...
                                            opt.n_layers_D, opt.norm, use_sigmoid, 
                                            opt.init_type, self.gpu_ids)
            self.netCLS = build_model('Deeplabv3plus_res50', 6)

            if opt.fs_model:
                if os.path.isfile(opt.fs_model):
                    checkpoint = torch.load(opt.fs_model)['model']
                    ...
                    if 'module.' in check_list[0][0]:
                        ...
                        for k, v in checkpoint.items():
                            new_stat_dict[k[7:]] = v
                        self.netCLS.load_state_dict(new_stat_dict, strict=True)
                    # Read the training weight of a single card, and continue training with a single card this time
                    else:
                        self.netCLS.load_state_dict(checkpoint)
                else:
                    print("no fs net found at '{}'".format(opt.fs_model))
                    raise FileNotFoundError("no fs net found at '{}'".format(opt.fs_model))
                self.netCLS.cuda()
    ...
    def set_input(self, input):
        ...
        if 'A_label' in input and 'B_label' in input:
            self.input_A_label = input['A_label' if AtoB else 'B_label'].to(self.device).long()
