...
class SenmanticData(Dataset):
    ...
    def __init__(self, root, augmentation=False, transform=None, target_transform=None):
        """
        Args:
...
class AddaDataLoader(object):
    def __init__(self, net_transform, dataset, datafolder, rootdir, downscale, crop_size=None,
            batch_size=1, shuffle=False, num_workers=2, half_crop=None, augmentation =False):
        ...
        assert len(self.datafolder)==2, 'Requires two datasets: source, target'
        sourcedir = os.path.join(rootdir, dataset,self.datafolder[0])
        targetdir = os.path.join(rootdir, dataset,self.datafolder[1])

        # load scr and tgt set
        self.source = SenmanticData(sourcedir, augmentation=augmentation)
        self.target = SenmanticData(targetdir, augmentation=augmentation)
