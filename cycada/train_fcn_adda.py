...
# @click.option('--dataset', default=['test_drone_sim', 'test_drone_real'], required=True, multiple=True)
# @click.option('--datadir', default="/home/shanci/Desktop/VNAV_sp/visnav_semantics/baseline/datasets/EPFL", type=click.Path(exists=True))
...
# @click.option('--discrim_feat/--discrim_score', default=True)
# @click.option('--weights_shared/--weights_unshared', default=True)
...
def main(args):
    ...
    train_discrim_only = args.train_discrim_only
    weights_discrim = args.weights_discrim
    crop_size = args.crop_size
    weights_shared = args.weights_shared
    discrim_feat = args.discrim_feat
    half_crop = args.half_crop
    batch = args.batch
    model = args.model
    ...
    logdir = 'runs/{:s}/{:s}/{:s}_to_{:s}/lr{:.1g}_ld{:.2g}_lg{:.2g}'.format(model, dataset, datafolder[0],
            datafolder[1], lr, lambda_d, lambda_g)
    if weights_shared:
    ...
    loader = AddaDataLoader(net.transform, dataset, datafolder, datadir, downscale,
...
def parse_args():
    ...
    parser.add_argument('--datafolder', type=str, default=['test_drone_sim', 'test_drone_real'],
                        help="dataset folder name for sim and real data")
    parser.add_argument('--datadir', default="/home/shanci/Desktop/VNAV_sp/visnav_semantics/baseline/datasets",
    ...
    parser.add_argument('--cls_weights', default=None,
                        help="directory to load the weights of classes")
    ...
    parser.add_argument('--weights_init', default='/media/shanci/Samsung_T5/cyclegta/drn26-test_drone_sim-iter115000.pth',
                        help="whether use real data in training")
    parser.add_argument('--model', type=str, default="drn26", choices=models.keys(), help="model name")
    parser.add_argument('--lsgan', type=bool, default=True)
    parser.add_argument('--num_cls', type=int, default=19, help="number of classes")
    ...
    parser.add_argument('--discrim_feat', type=bool, default=False)
    parser.add_argument('--weights_shared', type=bool, default=False)
    ...
    resdir = "/media/shanci/DataDisk/cyclegta/results/" + str(args.datafolder[0]) + "_to_" + str(args.datafolder[1]) + \
