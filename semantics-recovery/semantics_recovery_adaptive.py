...
def check_mem(sc_cdist_len: int, pc_cdist_len: int) -> bool:
    """
...
def find_opt_split(_sc: np.ndarray, _big_pc_label_tensor: torch.Tensor,
                   block_h: int, block_w: int) -> (int, int):
    """
    ...
    while not check_mem(sc_cdist_len, pc_cdist_len):
...
def main():
    ...
    for idx_dp, file_name in tqdm(enumerate(file_ls[start_idx:end_idx])):
        ...
        block_h, block_w = find_opt_split(_sc, _big_pc_label_tensor, block_h, block_w)
